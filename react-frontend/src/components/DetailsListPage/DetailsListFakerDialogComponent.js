...
import detailsListFakerFactory from "./detailsListFakerFactory";
...
const DetailsListFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = detailsListFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("detailsList").create(elem));
...
export default connect(null, mapDispatch)(DetailsListFakerDialogComponent);
