...
const ScdCTCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        const result = await client.service("scdCT").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info scdCT updated successfully" });
    ...
    return (
        <Dialog header="Edit Info" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div role="scdCT-edit-dialog-component">
                <div>
                <p className="m-0">Description:</p>
                <Dropdown value={_entity?.itemDescription} options={itemDescriptionOptions} optionLabel="name" optionValue="value" onChange={(e) => setValByKey("itemDescription", e.value)} />
            </div>
            <div>
