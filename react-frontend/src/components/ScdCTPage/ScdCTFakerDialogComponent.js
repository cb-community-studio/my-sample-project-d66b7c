...
import scdCTFakerFactory from "./scdCTFakerFactory";
...
const ScdCTFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = scdCTFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("scdCT").create(elem));
...
export default connect(null, mapDispatch)(ScdCTFakerDialogComponent);
